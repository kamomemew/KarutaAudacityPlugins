...
$name (_ "Karuta Yomi Labeling (original: Label Sounds)")
$debugbutton false
$author (_ "kamome (original: Steve Daulton)")
$release 0.0.1
$copyright (_ "GNU General Public License v3.0 or later")
    ...
    (do ((i (- (length snd-list) 2) (1- i)))
      ...
      (case TYPE
        (0  ;;label silences.
            ...
            (setf next-start-time (first (nth (1- i) snd-list)))
            (setf shimo (> (- start-time prev-end-time) 3.5))
            (setf kami (> (- next-start-time end-time) 3.5))
            (setf orphan (and shimo kami))
            (setf pre-txt (cond  (orphan "noise")(kami "kami")(shimo "shimo")((string-search "kami" prev-text) "*shimo")((string-search "shimo" prev-text) "*kami" )((string-search "noise" prev-text) "*shimo" )(t "?") ))
            (setf prev-text pre-txt)
